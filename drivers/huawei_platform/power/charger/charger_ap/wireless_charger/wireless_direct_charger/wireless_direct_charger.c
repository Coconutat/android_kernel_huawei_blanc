...
static int wldc_get_imax_by_ntc_fit(struct wldc_dev_info *di)
{
	int ntc_fit_plimit;
	int vmax;
	int rx_ratio;

	ntc_fit_plimit = WLDC_PLIM_BY_NTC_FIT;
	rx_ratio = di->mode_para[di->cur_dc_mode].init_para.rx_ratio;
	vmax = WLDC_VBAT_OVP_TH * rx_ratio * di->volt_ratio;
	if (vmax > 0)
		return ntc_fit_plimit / vmax;
...
static void wldc_select_charge_param(struct wldc_dev_info *di)
{
	...
	if (hisi_coul_ntc_fitted_iqr()) {
		ntc_fit_ilimit = wldc_get_imax_by_ntc_fit(di);
		di->cur_cp_iout_hth = di->cur_cp_iout_hth > ntc_fit_ilimit ?
			ntc_fit_ilimit : di->cur_cp_iout_hth;
	}
}
