...
bool pogopin_3pin_ignore_pogo_vbus_in_event(void)
{
	...
	ret = vbus_ch_get_state(VBUS_CH_USER_POGOPIN, VBUS_CH_TYPE_POGOPIN_BOOST, &vbus_ch_state);
	if (ret) {
...
static void pogopin_3pin_pogo_vbus_out(void)
{
	...
	if (di->charge_otg_ctl) {
		vbus_ch_get_state(VBUS_CH_USER_POGOPIN, VBUS_CH_TYPE_POGOPIN_BOOST, &vbus_ch_state);
	...
	struct device_node *np)
{
	...
	(void)power_dts_read_u32_compatible("huawei,vbus_channel_pogopin_boost",
